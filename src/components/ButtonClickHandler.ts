...
async function handleCheckRegistrationButtonClick(interaction: ButtonInteraction) {
  ...
  const registeredEmailAddress = await Bot.getInstance()
    .getStorage()
    .getRegisteredUserEmailAddress(interaction.user.id);
...
async function handleDeleteRegistrationButtonClick(interaction: ButtonInteraction) {
  ...
  console.log(
    `Deleted email address for <@${interaction.user.id}> @${interaction.user.username}#${interaction.user.discriminator}`,
  );
