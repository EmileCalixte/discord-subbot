...
async function handle(channel: TextChannel): Promise<HandleReturnData> {
  ...
  const { failed: failedToDeleteExistingMessage, messageUrl: deletedMessageUrl } = await deleteExistingMessageIfExists(
    channel.guildId,
  );
...
async function deleteExistingMessageIfExists(
  guildId: Snowflake,
): Promise<{ failed: boolean; messageUrl: string | null }> {
  ...
  execute: async (interaction) => {
    ...
    if (channel === null) {
      interaction.editReply(
        getLocaleString(interaction.locale, {
    ...
    try {
      const { failedToDeleteExistingMessage, deletedMessageUrl } = await handle(channel);
